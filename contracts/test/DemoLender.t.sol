...
contract DemoLenderTest is TestBase {
    ...
    function setUp() public {
        ...
        uint256 mask = bureau.permissionFor(PerformanceBureau.EvidenceKind.AaveSelfRepaymentObservation)
            | bureau.permissionFor(PerformanceBureau.EvidenceKind.SandwichBreach);
        bureau.setReporterPermissions(REPORTER, mask);
    }

    function test_VerifiedObservationProducesConservativeCollateralQuote() public {
        ...
        (PolicyV1.Terms memory afterTerms, uint128 requiredCollateral) = lender.quote(BORROWER, 100e6);
        assertEq(afterTerms.collateralBps, 14_500);
        assertEq(afterTerms.maxBorrowUsdc, 140e6);
        assertEq(requiredCollateral, 145 ether);
    }
    ...
    function test_OfferFreezesTermsAtOrigination() public {
        _record(bytes32(uint256(1)), PerformanceBureau.EvidenceKind.AaveSelfRepaymentObservation, 4_000e6, false);
        vm.warp(1_000_000);
        bytes32 offerId = lender.makeOffer(BORROWER, 100e6, 7 days);
        ...
        assertEq(offer.principalUsdc, 100e6);
        assertEq(offer.requiredCollateralCtc, 145 ether);
        assertEq(offer.terms.collateralBps, 14_500);
        assertEq(offer.expiresAt, 1_000_000 + 7 days);
        assertEq(current.collateralBps, 15_000);
